...
class GridMap {
    ...
    highlight_a_cell(state){
        if(state == "US"){return}
        //console.log(this._cells_object)

        this._cells_object
            .transition().duration(100)
            .attr("opacity", d => d.code== state ? 1 : 0.4);
            if (this._style.alwaysShowOverlay)
        this._cells_object.selectAll(".ltext").attr("opacity", d => d.code== state ? 1 : 0);
                    
        //this._cells_object.forEach( function(d) {
        ...
        //         if (d.total && d.values) this._showLegend(d);
        //         state_on_select(d.code)                            // out put information
        //     })

    }

    cancel_hightlight(){
        this._cells_object.transition().duration(100)
    ...
    _initColors() {
        if (this._chartData) {
            ...
            this._t = function(d){  var color=d3.rgb(this._c(d))
                                 ...
                                 //d3.scaleSequential(this._textPalette).domain(ext);
    ...
    _renderSquares(g, size) {
        ...
        const og = g.filter(d => d.total)
            ...
            .call(g => g.append("g")
                ...
                //TODO 
                .attr("font-size", "25pt")       // changed label text size
                .attr("transform", d => style.sizeByValue ?
                    `translate(${d.r - size},${d.r * 2 - size + 15})` :
                    `translate(0,${bandwidth.hx + 33})`) // changed y for label text position
                .call(g => {
    ...
    _addCellText(g, short, className, opacity) {
        var cell_text_size = "16pt"  // changed cell text size
        var cell_label_size = "30pt"    // changed label text size
        const tg = g.append("g")
            .attr("class", className)
            .attr("opacity", opacity)
            .attr("text-anchor", "middle")
            .style("font-weight", "bold")
            .attr("fill", d => d.total ? short ? this._t(d.total) : "white" : this._style.defaultTextColor)  //over-lay label text
            .attr("transform", `translate(${this._bandwidth.hx},${this._bandwidth.hy})`);

        if (short || this._showLabel) tg.call(g => g.append("text").text(d => short ? d.code : d.state).attr("font-size", d => short? cell_text_size: cell_label_size) ); // changed size
        tg.call(g => g.append("text")
            .attr("dy", short || this._showLabel ? "1em" : "") 
            .attr("font-size", d => short? cell_text_size : cell_label_size)   // label number size
            .text(d => d.total ? d3.format(short ? (d.total>=1000? this._style.shortFormat : d.total) : this._style.longFormat)(d.total) : "---"));
    }
    ...
    _renderLegend(g) {
        ...
        if (s.length > 0) {
            g.attr("font-weight", "bold")
                ...
                .attr("dy", "-.3em")
                .attr("transform", `translate(0,3)`) //`translate(${w * 9},3)`
        ...
        return g.selectAll("g")
            ...
            .attr("font-family", this._style.font)
            .style("font-weight", "bold")
            .attr("transform", (d, i) => `translate(${i * w},5)`)
            .call(g => g.append("rect")
                .attr("y", "1.2em")
